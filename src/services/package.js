...
export const PackageData = async (req) => {
  ...
  const ItemData = await PackageModelSchema.create({
    name,
    description,
    price,
    isDelete: false,
    PackageImage: req.file ? req.file.path : null,
  });
  return ItemData;
};
...
export const getPackageData = async () => {
  ...
  const ItemData = await PackageModelSchema.aggregate([
    { $match: condition_obj },
    {
      $addFields: {
        imageUrl: {
          $ifNull: [{ $concat: [image_url.url, "$PackageImage"] }, ""],
        },
      },
    },
  ]);
...
export const UpdatedPackage = async (req) => {
  ...
  if (req.file) AddItem.PackageImage = req.file.path;
...
export const deletedPackage = async (req) => {
  ...
  const findAddItem = await PackageModelSchema.findByIdAndUpdate(id, {
    isDelete: true,
  });
